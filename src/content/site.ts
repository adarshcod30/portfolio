...
export const EXPERIENCE = [
  {
    ...
    bullets: [
      "Compared three ways to run LangGraph agents in production and chose Aegra, the open-source Agent Protocol server, because the commercial platform would move agent state outside Deloitte and an in-house runtime would rebuild a mature tool.",
      "Built capability in five layers on a multi-agent travel planner, from a single agent to a supervisor with three specialists, a human approval gate, real browser tools over MCP and serving through Aegra.",
      "Replaced request-time graph assembly with a graph factory that registers one static graph and carries per-request variation in run state, verifying the migrated output identical to the original field by field.",
      "Kept the public API unchanged so the front end and test collections needed no change, and made the migration reversible through a single setting, with the original execution path kept byte for byte.",
      "Proved it on the live SAP tenant: a Bank Accounting goal became 8 configuration tasks, run in a real browser through Playwright MCP, recovering from a duplicate entry and a mis-filled field to finish 8 of 8 in about 15 minutes.",
      "Worked out a Docker-free deployment on a shared Linux server and fixed failures that raised no error, from two agent versions colliding on one module name to a stream reading a field the SDK never returns.",
    ],
    stack: ["LangGraph", "LangChain", "Aegra", "Agent Protocol", "Playwright MCP", "FastAPI", "PostgreSQL", "Python 3.13", "Linux", "Human-in-the-loop"],
